...
func (s *UserInfoSuite) TestValidateUserInfo() {
	...
	tests := []struct {
	...
	}{
		{
			"empty info",
			fields{s.conf},
			args{user.NewInfo(nil, nil, nil, nil, nil)},
			true,
		},
		{
			"id",
			fields{s.conf},
			args{user.NewInfo(&ids[0], nil, nil, nil, nil)},
			true,
		},
		{
			"username",
			...
			args{user.NewInfo(nil, &usernames[0], nil, nil, nil)},
			true,
		},
		{
			"email",
			fields{s.conf},
			args{user.NewInfo(nil, nil, &emails[0], nil, nil)},
			true,
		},
		{
			"password",
			fields{s.conf},
			args{user.NewInfo(nil, nil, nil, &passwrods[0], nil)},
			true,
		},
		{
			"blocked until",
			fields{s.conf},
			args{user.NewInfo(nil, nil, nil, nil, &blocks[0])},
			true,
		},
	}
